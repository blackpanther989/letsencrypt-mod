...
class _RFC2136Client(object):
    ...
    def add_txt_record(self, record_name, record_content, record_ttl):
        ...
        if (_RFC2136Client.update is None):
            _RFC2136Client.update = dns.update.Update(
            domain,
            keyring=self.keyring,
            keyalgorithm=self.algorithm)
        _RFC2136Client.update.add(rel, record_ttl, dns.rdatatype.TXT, record_content)


        try:
            response = dns.query.udp(self.update, self.server, port=self.port)
